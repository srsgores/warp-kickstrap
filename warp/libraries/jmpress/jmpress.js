...
(function ($, document, window, undefined)
{
	...
	$.jmpress("setActive", function (element, eventData)
	{
		...
		for (var i = eventData.current.maxNestedDepth;
		     i > (eventData.parents.length || 0);
		     i--)
		{
...
(function ($, document, window, undefined)
{
	...
	{
		for (var variable in
		{"viewPortHeight": 1, "viewPortWidth": 1, "viewPortMinScale": 1, "viewPortMaxScale": 1, "viewPortZoomable": 1})
		{
...
(function ($, document, window, undefined)
{
	...
	$.jmpress("defaults").duration = {
		defaultValue: -1, defaultAction: "next", barSelector: undefined, barProperty: "width", barPropertyStart: "0", barPropertyEnd: "100%"
	};
	$.jmpress("initStep", function (step, eventData)
	{
	...
	$.jmpress("setInactive", function (step, eventData)
	{
		...
		if (dur && dur > 0)
		{
			...
			{
				var css = {
					transitionProperty: durationSettings.barProperty, transitionDuration: '0', transitionDelay: '0', transitionTimingFunction: 'linear'
				};
	...
	$.jmpress("setActive", function (step, eventData)
	{
		...
		if (dur && dur > 0)
		{
			...
			{
				var css = {
					transitionProperty: durationSettings.barProperty, transitionDuration: (dur - settings.transitionDuration * 2 / 3 - 100) + "ms", transitionDelay: (settings.transitionDuration * 2 / 3) + 'ms', transitionTimingFunction: 'linear'
				};
